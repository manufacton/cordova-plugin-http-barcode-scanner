/********* httpBarcodeScanner.m Cordova Plugin Implementation *******/
...
@interface HttpBarcodeScanner : CDVPlugin {
...
- (void)getQr:(CDVInvokedUrlCommand*)command;
...
#define IP_LOCALHOST        @"localhost"
#define IP_ANY              @"0.0.0.0"

@implementation HttpBarcodeScanner
...
- (void)getQr:(CDVInvokedUrlCommand*)command
{
    ...
    pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:[self.cameraManager getQrCode]];
...
- (void)startCamera:(CDVInvokedUrlCommand*)command
{
	NSLog(@"IN CAMERA START");
	[self startServer:command];
	NSLog(@"OUTSIDE SERVER START");
